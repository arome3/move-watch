...
export function EventStream({
  ...
  const { data, isLoading, error, dataUpdatedAt } = useQuery({
    queryKey: ['events', network, moduleAddress, eventType, sender],
    queryFn: () =>
      fetchEvents(network, {
        moduleAddress,
  ...
  return (
    <div className="bg-dark-800 rounded-lg border border-dark-700 flex flex-col h-[500px]">
      {/* Header */}
  ...
  return (
    <>
      {[1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="p-4 bg-dark-900 rounded-lg border border-dark-700 animate-pulse"
        >
          <div className="flex items-center justify-between">
            <div className="h-4 w-48 bg-dark-700 rounded" />
            <div className="h-3 w-24 bg-dark-700 rounded" />
          </div>
          <div className="mt-3 h-16 bg-dark-800 rounded" />
        </div>
      ))}
    </>
  );
