...
export function TransactionHistory({
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['transactions', network, moduleAddress, status, search, offset],
    queryFn: () =>
        ...
        <div className="p-4 border-t border-slate-700 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1.5 text-sm font-medium text-slate-300 bg-slate-700 rounded-lg hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Previous
            </button>
            <button
      ...
      {[1, 2, 3, 4, 5].map((i) => (
        <div key={i} className="p-4 animate-pulse">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block h-4 w-48 bg-slate-700 rounded" />
